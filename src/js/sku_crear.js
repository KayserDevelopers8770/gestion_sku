...
$(document).ready(function() {
  ...
  art_existente='nuevo'; //CUANDO SE RENDERIZA LA PAGINA, EL PRIMER ARTICULO QUE SE INGRESE SERA UNO NUEVO, SI DESPUES SE AGREGAN MAS ARTICULOS A LA LISTA NUEVA, ESTOS SERAN EXISTENTES
  name_dpto='dama';
  ...
  if(initial_option == 'show'){
    ...
    console.log('CALL API / file: sku_lista.php'); console.log('REQUEST:', parameters);
...
/*******************************************************************************************************/
  ...
  function getObjectArticle(tipo_arti){
    ...
    tipo_arti=='existente' ? code_article=art_existente : code_article = el_txt_prefijo.value + el_txt_correlativo.value + el_txt_sufijo.value;          
    ...
    //SOLO SI EL ARTICULO ES NUEVO, AGREGAMOS LOS DATOS PRINCIPALES NO MODIFICABLES
    if(tipo_arti=='nuevo'){
    ...
    console.log('Articulo preparado antes de enviar a la api: ',obj_article);
    return obj_article;
  }
  ...
  function getPrefix(values){
    ...
    $.ajax({ url : './models/sku_prefijo.php', type : 'post', dataType : 'json', data : values,
      ...
      success : function(data) {
        ...
        // el_div_loader_full.classList.remove('cont_hidden');
        if(!!data['errors']){
          console.log("Error al consultar PREFIJO, en consulta o Conexion a BDx: ", data['errors']);          
        }else {       
...
function ajaxFillSelects(param){
  console.log('CALL API to fill inputs when article exist in list / file: sku_crear.php')
  ...
  $.ajax({    
    ...
    success: function (data) {
      ...
      if(data.cant_skus==0){
        ...
        ///--- llenamos la tabla skus existentes provenientes de sap, para que reporten que es lo que ya esta registrado
        if(!!data.skus){
        ...
        ///--- LLENAMOS LOS SELECT CON  LOS OPTIONs ENVIADOS DESDE LAS APIS       
        if(!!data.selects){
          cant_selects=data.selects.length;
          for(let i=0; i<cant_selects;i++){
            if (!!document.getElementById('select_sku_' + data.selects[i].select)){
              // if (data.selects[i].select)    
              // $("select[name='" + data.selects[i].select + "']").html(data.selects[i].options);
              // console.log(document.getElementById('select_sku_' + data.selects[i].select));
              document.getElementById('select_sku_'+ data.selects[i].select ).innerHTML = data.selects[i].options
              if (data.selects[i].select=='composicion'){
                 $('#select_sku_composicion').selectpicker('refresh')
                // let pos_selected = data.selects[i].options.indexOf("' selected"); //OBTENEMOS LA POSICION DE LA CADENA  [' selected]
                // if(pos_selected != -1 ){
                //   let pos_value = data.selects[i].options.substr(0,pos_selected-1).lastIndexOf("'");
                //   let value_option = data.selects[i].options.substr(pos_value + 1 , pos_selected - 1 )
          ...
          //SI EL ARTICULO EXISTE EN SAP
          if(param.origin == 'sap') {
            el_div_operation_title.innerHTML = "CREACION DE SKUS PARA ARTICULO " + art_existente + "EXISTENTE EN SAP"
            document.querySelectorAll('.sku_control').forEach(function (ctrl) { //DESHABILITA TODOS LOS CONTROLES QUE TENGAN LA CLASE .sku_control
              ctrl.disabled = true;
            });            
            //mala practica, pero a continuacion volvemos ha habilitar algunos de esto constroles 
            el_sel_color.disabled = false;
            el_sel_copa.disabled = false;
            el_sel_fcopa.disabled = false;
            //INICIALMENTE NO PERMITIA EDITAR LOS DETALLES DESPUES DELOS FORMULARIOS COPA, pero despues solicitaron que debemos habilitarlos
            el_sel_tprenda.disabled = false;
            el_sel_tcatalogo.disabled = false;
            el_sel_grupouso.disabled = false;
            el_sel_caracteristica.disabled = false;
            el_sel_composicion.disabled = false;
...
function ajaxAddArticleList(param){
  console.log('CALL API to save article in list / file: sku_lista.php');
  console.log('REQUEST: ',param)
  $.ajax({
    url: './models/sku_lista.php', type: 'post', dataType: 'json', data: param,
    beforeSend: function () { el_div_loader_full.classList.remove('cont_hidden'); },
    success: function (data) {
      console.log('RESPONSE: ',data);
      el_div_loader_full.classList.add('cont_hidden');
      console.log('FROM API (option: ' + param.option + ' file api: sku_lista.php ) ', data);
      if (!!data.nothing) {
        alert(data.nothing);
      } else {
...
function renderArticleList(art, itn, rows, detail, estado_article, existencia) { // el estado_article = ESTADO EN LISTA MOSTRADA, ES DECIR SI EL ARTICULO YA ESTA EN LISTA, SOLO AGREGAREMOS LOS NUEVOS SKUS
    ...
    but_del.onclick = function () {
      if (confirm('¿DESEA QUITAR ESTE ARTICULO DE LA LISTA?')) {
        ...
        $.ajax({
          ...
          beforeSend: function () { },
          success: function (data) {
            console.log('from api: sku_lista ' + data);
  ...
  document.querySelectorAll('.btn_edit_detalle, .btn_add_color_talla').forEach(function (but_view_edit) {
    but_view_edit.onclick = function () {    
      alert('ESTAMOS IMPLEMENTANDO ESTA OPCION. Gracias por su comprensión... ')  
