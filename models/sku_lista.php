...
if($_POST['option']=="save_article"){
  ...
  if($existencia=='sap'){
    ///TENEMOS QUE OBTENER ALGUNA INFORMACION DE SAP
    ...
    C.Name as categoria_name, S.U_Marca AS marca_name, S.U_FILA as presentacion_name, S.U_Material as material_name, S.CodeBars as barcode, S.U_IDCopa as copa_name, S.U_GSP_SECTION as forma_copa, S.U_EVD as tprenda_name, S.U_APOLLO_SEG2 as color, S.U_APOLLO_S_GROUP as tcatalogo_name, 
    S.U_ESTILO as grupouso_name, S.U_APOLLO_COO as composicion_name, S.FrgnName as caracteristica_name, S.U_APOLLO_SEG3 as talla_familia FROM OITM AS S 
    ...
    WHERE (S.U_APOLLO_SEG1 IS NOT NULL) AND s.ItemCode like '$code_article-%'";
    ...
    if($arr_articulo!==false && $arr_articulo!==0){
      ...
      $query_insert_article.="'".$_POST['talla_familia']."','sap')"; 
...
if($_POST['option']=="save_list") {
  ...
  if($reg_updated==1){
    ...
    $destinatario = ( $env=='dev' ) ? 'aobando@outlook.cl' : 'sku@kayser.cl';
    $headers = "MIME-Version: 1.0\r\n"; 
    $headers .= "Content-type: text/html; charset=UTF-8\r\n"; //PARA ENVIO EN FORMATO HTML
    $headers .= "From: CREACION DE SKUs <informatica@kayser.cl>\r\n";
...
if($_POST['option']=='finalize_list'){
  ...
  if($arr_skus!==false && $arr_skus!==0){
    ...
    $destinatario = ( $env=='dev' ) ? 'aobando@outlook.cl' : 'sku@kayser.cl';
    $headers = "From: FINALIZACION DE CREACION DE SKUs <informatica@kayser.cl>\r\n";
    $headers .= "MIME-Version: 1.0\r\n";
...
function sendMail($arr_cont){
  ...
  $header .= "From: REVISION DE SKUs <informatica@kayser.cl>\r\n";
